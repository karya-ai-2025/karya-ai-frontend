            ...
            {pricingTiers.map((tier, i) => (
              <div key={i} className={`group relative animate-fadeInUp ${tier.popular ? 'z-10' : ''}`} style={{ animationDelay: `${i * 150}ms` }}>
                ...
                <div className={`relative bg-white border rounded-2xl sm:rounded-3xl p-5 sm:p-6 h-full ${tier.popular ? 'border-indigo-500 shadow-lg' : 'border-gray-200 hover:border-indigo-200'} transition-all`}>
                  ...
                  <button onClick={() => router.push('/register')} className={`w-full py-2.5 sm:py-3 rounded-lg sm:rounded-xl font-semibold text-sm sm:text-base transition-all ${tier.popular ? 'bg-indigo-600 text-white hover:shadow-lg hover:shadow-indigo-500/20' : 'bg-gray-50 hover:bg-gray-100 text-gray-900 border border-gray-300'}`}>
                    {tier.cta}
                  </button>
                </div>
              </div>
